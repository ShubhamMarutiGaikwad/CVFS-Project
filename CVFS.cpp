...
#define MAXINODE 5  // number of file can be created
...
#define REGULAR 1  // type of file we can create
#define SPECIAL 2  // ex, folder
//below are offset
#define START 0
#define CURRENT 1
#define END 2
...
UFDT UFDTArr[50];  //array of above struct
SUPERBLOCK SUPERBLOCKobj;  //object of superblock struct
PINODE head=NULL;
...
void man(char *name)
{
    ...
    if(strcmp(name,"create")==0)
    {
        ...
        printf("Usage: create File_name ermission\n");
    }
    ...
    {
        printf("Description: Used to write data from file\n");
        printf("Usage: write File_name\n After this enter the data that you want to write\n");
    }
    else if(strcmp(name,"Is")==0)
    {
        printf("Description: Used to lis all the information of files\n");
        printf("Usage: ls\n");
    }
...
void DisplayHelp()
{
    ...
    printf("closeall : To close the all opened file\n");
...
int GetFDFromName(char *name)
{
    ...
    while(i<50)
    {
        ...
        { if(strcmp((UFDTArr[i].ptrfiletable->ptrinode->FileName),name)==0)  // curley bracket
            break;    
        }i++;
    }
...
int fstat_file(int fd)
{
    ...
    if(temp->permission==1)
         ...
         {printf("File Permission:Read + Write n");}
    printf("---------------------------------------\n\n");
...
int main()
{
    ...
    while(1)
    {
        ...
        if(count==1)
        {
            ...
            {
                printf("Terminating the Marvellous Virtuak File System\n");
                continue;
            }
            else
            {
        ...
        else if(count==3)
        {
           if(strcmp(command[0],"create")==0)
           {
            ...
            if(ret>=0)
               {printf("File is sucessfully created with descripter: %d\n",ret);}
            if(ret==-1)
               {printf("ERROR: Incorrect parameter\n");}
            if(ret==-2)
            ...
            if(ret>=0)
               {printf("File is sucessfully opened with descripter: %d\n",ret);}
            if(ret==-1)
               {printf("ERROR: Incorrect parameter\n");}
            if(ret==-2)
